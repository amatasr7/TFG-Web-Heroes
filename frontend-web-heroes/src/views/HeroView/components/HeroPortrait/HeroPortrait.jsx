...
const spriteMap = {
  guerrero: "/sprites/Guerrero2.png",
  mago: "/sprites/Maga.png",
  picaro: "/sprites/Arquero.png",
  jefe: "/sprites/Orco.png",
};

function getSpriteFromClass(clase) {
  if (!clase) return spriteHeroeDefault;
  ...
  return spriteHeroeDefault;
}

function getEmblemaFromClass(clase) {
  if (!clase) return null;
  const normalized = clase.toLowerCase();

  if (normalized.includes("guerrero")) return EMBLEMA_ICONS.guerrero;
  if (normalized.includes("mago")) return EMBLEMA_ICONS.mago;
  if (normalized.includes("picaro") || normalized.includes("pícaro")) return EMBLEMA_ICONS.picaro;

  return null;
}

const EmblemaClaseFondo = ({ clase }) => getEmblemaFromClass(clase);
